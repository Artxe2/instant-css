...
const copyChilds = (container: HTMLElement, childElements: NodeListOf<HTMLElement>) => {
	...
	while (container.scrollWidth < container.clientWidth + maxChildWidth * 3) {
		childElements.forEach(child => {
...
<div bind:this={container} on:mousedown={handleMousedown} on:touchstart={handleTouchstart} on:touchmove={handleTouchmove} on:touchend={handleTouchend}
	class="flex o=scroll ::-webkit-scrollbar/none touch-action=none {classs}"
>
	<slot></slot>
</div>